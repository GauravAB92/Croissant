...
//TODO:
	//Use logger:: to log errors instead of MessageBoxA
...
inline std::string HrToStringVerbose(HRESULT hr)
{
    ...
    std::string message = (size > 0) ? msgBuffer : "Unknown error";
    LocalFree(msgBuffer);
    return message;
}
...
inline void ThrowIfFailed(HRESULT hr)
{
    if (FAILED(hr))
    {
		MessageBoxA(nullptr, HrToStringVerbose(hr).c_str(), "Error", MB_OK | MB_ICONERROR);
